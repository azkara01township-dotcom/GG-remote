...
local dev = os.date("┇💎﹝A R H   S C R I P T﹞💎\n┇👑 Azka Raditya Hermawan\n┇📅 %A, %d %B %Y | ⏰ %I:%M %p")
 ...
 -- 🛡️ ARH SECURE SYSTEM v1.2 (Optimized Version)  
...
-- 🔄 Safe Request (tidak block & tidak error)  
...
-- 🌐 Server Date from Google (fast, safe)  
local function getServerDate()  
  local r = safeRequest("http://www.google.com")  
...
-- 🌍 Get IP & Location (non-blocking, cepat)  
local function getIPData()  
  local res = safeRequest("http://ip-api.com/json?fields=status,country,city,isp,query")  
  if not res or not res.content then  
    return { ip="Unknown", country="Unknown", city="Unknown", isp="Unknown" }  
  end  
  return {  
    ip      = res.content:match('"query":"(.-)"') or "Unknown",  
    country = res.content:match('"country":"(.-)"') or "Unknown",  
    city    = res.content:match('"city":"(.-)"') or "Unknown",  
    isp     = res.content:match('"isp":"(.-)"') or "Unknown"  
  }  
end  
...
-- 📊 Tracking Pengguna (optimized, async)  
...
-- ✅ Inisialisasi Awal (lebih ringan & cepat)  
checkTime()               -- proteksi waktu (1x)  
resetUserLogMonthly()     -- reset log kalau tanggal 1  
gg.toast(_("connecting"))  
gg.sleep(50)              -- lebih cepat tampil menu  

-- 📌 Jalankan logging di background (async, non-blocking)  
pcall(trackAndLog)
---------------------------------------------------------------------------------------------------------
-- 🌐 Bahasa
